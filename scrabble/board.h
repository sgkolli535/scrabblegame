...
public:
    ...
    struct Anchor {
        ...
        Anchor(Position p, Direction d, size_t l) : position(p), direction(d), limit(l) {}
    };
    ...
    /*
    The goal of test_place() is to check what would happen if a given Move was executed on the board. It should not
    actually modify the board in any way. It should fill in a PlaceResult with, quoting the assignment, "whether the
    move is valid, what words would be generated, how many points the move would be worth... or if not valid, what error
    message to display"

    So, first and foremost, test_place() should determine if the given move is valid to execute on this board. If it is
    not valid, then return a PlaceResult with valid set to false, and set the error field to a human-readable error
    message that shows the user what they did wrong (there are no specific requirements for this error message as long
    as it describes the error). On the other hand, if the move is valid, then test_place() should set valid to true and
    the list of all created words should be returned in the words field of the PlaceResult. It should also total up all
    points scored by the move and return those in the PlaceResult's points field (see the Scoring section of the
    assignment).

    NOTE: test_place() does not (and can't) check if the words formed are actually in the dictionary. Checking if words
    are valid is the responsibility of the caller of test_place()!
    */
    PlaceResult test_place(const Move& move) const;

    PlaceResult place(const Move& move);  // Used for testing - remember that the move struct should use 0 based
                                          // indexing, NOT 1 based
